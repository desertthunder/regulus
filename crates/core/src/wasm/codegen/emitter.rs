...
use super::{DebugImport, DecodeLocals, FunctionSignature, PatternSubject, StructuredError, StructuredResult};
...
use crate::wasm::{constructor_tag, runtime_helper_bundle};
use crate::{ClosureConstants, abi::STDLIB_IO_HOST_MODULE, runtime, types::Type};

use super::helpers::*;

pub struct StructuredEmitter<'a> {
    source: &'a ir::Module,
    module: Module,
...
impl<'a> StructuredEmitter<'a> {
    pub fn new(source: &'a ir::Module, options: EmitOptions) -> Self {
        Self {
            source,
            module: Module::new(),
    ...
    pub fn module(mut self, source: &ir::Module) -> StructuredResult<Module> {
        ...
        if !self.runtime_helper_roots.is_empty() {
            let helper_bundle =
                runtime_helper_bundle(self.config, &self.runtime_helper_roots).map_err(StructuredError::Invariant)?;
    ...
    fn emit_js_host_abi_helpers(&mut self) {
        ...
        // TODO: this should be a helper as an include_str! wat file
...
"#
    ...
    fn constructor_value(
        ...
        out.push(Instruction::I32Store(MemoryArg::new(self.ensure_memory(), 4, 2)));
        out.push(Instruction::LocalGet { local: ptr, type_: ValueType::I32 });
        out.push(Instruction::I32Const(constructor_tag(&constructor.name) as i32));
        out.push(Instruction::I32Store(MemoryArg::new(self.ensure_memory(), 8, 2)));
    ...
    fn record_update(
        ...
        if !matches!(type_, Type::Record { .. }) {
            out.push(Instruction::LocalGet { local: ptr, type_: ValueType::I32 });
            out.push(Instruction::I32Const(constructor_tag(constructor) as i32));
            out.push(Instruction::I32Store(MemoryArg::new(self.ensure_memory(), 8, 2)));
        }
        for (index, field) in fields.iter().enumerate() {
            out.push(Instruction::LocalGet { local: ptr, type_: ValueType::I32 });
    ...
    fn pattern_test_subject(
        ...
        match pattern {
            ...
            ir::IrPattern::Constructor { name, arguments } => {
                ...
                out.push(Instruction::I32Const(constructor_tag(name) as i32));
